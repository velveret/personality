...
def train(positiveFile='mixedSmiley2/positive.csv', negativeFile='mixedSmiley2/negative.csv', nOccurrences=25, trainProportion=0.9):
  ...
  bandkeywords = ["local","natives","born","ruffians","robert","francis"]
  for f in masterfeats:
    if masterfeats[f] < nOccurrences or f in bandkeywords:
      del mfn[f]
  masterfeats = mfn
  f = open("features-mixed2-2.lst", "w")
  ...
  f = open("classifier-mixed2-2.pickle", 'w')
  pickle.dump(classifier, f)
  f.close()
  f = open("features-mixed2-2.pickle", 'w')
